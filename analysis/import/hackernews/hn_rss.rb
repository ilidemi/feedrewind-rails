...
def hn_rss(input_rows, throttler, logger)
  ...
  input_rows.each do |input_row|
    ...
    logger.debug("[[#{index}]] Url #{url}, sum_score #{sum_score}, count #{count}")
    ...
    begin
      ...
      Net::HTTP.start(
        ...
        prefix_urls_curls.each do |prefix_url_curl|
          logger.debug("Trying prefix #{prefix_url_curl.url}")
          ...
          loop do
            ...
            if resp.code.start_with?('3')
              ...
              if redirection_link.nil?
                logger.debug("#{resp.code} #{prefix_url_curl.url} -> bad redirection link")
                break
              end

              if seen_urls.include?(redirection_link.url)
                logger.debug("#{resp.code} #{prefix_url_curl.url} -> #{redirection_link.url} - infinite redirect")
                break
              end
              seen_urls << redirection_link.url

              logger.debug("#{resp.code} #{prefix_url_curl.url} -> #{redirection_link.url}")
              prefix_uri = redirection_link.uri
              next
            elsif resp.code == "200"
              logger.debug("#{resp.code} #{prefix_url_curl.url}")

              content_type = resp.header["content-type"]&.split(";")&.first
              unless content_type == "text/html"
                logger.debug("Not an html")
                break
              end
              ...
              feed_links.each do |feed_link|
                logger.debug("Feed found: #{prefix_uri.to_s} -> #{feed_link.url}")
              ...
              logger.debug("#{resp.code} #{prefix_url_curl.url} - some error")
              break
            end
          end
...
def find_feed_links(page_doc, page_uri, throttler, http, http_host, http_port, logger)
  ...
  if probable_feed_links.length > 10
    logger.debug("Too many probable feeds (#{probable_feed_links.length}")
    return []
  end

  logger.debug("Checking #{probable_feed_links.length} probable feeds")
    ...
    begin
      ...
      loop do
        ...
        if resp.code.start_with?('3')
          ...
          if redirection_link.nil?
            logger.debug("Feed #{resp.code} #{feed_link.url} -> bad redirection link")
            break
          end

          if seen_urls.include?(redirection_link.url)
            logger.debug("Feed #{resp.code} #{feed_link.url} -> #{redirection_link.url} - infinite redirect")
            break
          end
          seen_urls << redirection_link.url

          logger.debug("#{resp.code} #{feed_link.url} -> #{redirection_link.url}")
          ...
          logger.debug("Feed not 200: #{resp.code} #{feed_link.url} (#{page_uri})")
          break
        end
      end
      next unless content
      ...
      logger.debug("Is#{is_feed ? "" : " not"} feed: #{feed_link.url}")
      is_feed
    rescue => e
      logger.debug(e)
      next
