...
def extract_title_relative_xpaths(
  ...
  neighbor_xpaths.each do |neighbor_xpath|
    ...
    if are_all_neighbor_titles_matching
      matching_neighbor_xpaths << TitleRelativeXpath.new(neighbor_xpath, :neighbor)
    end
  end

  unless matching_neighbor_xpaths.empty?
    log_lines << "neighbor titles matching at [#{matching_neighbor_xpaths.map(&:to_s).join(", ")}]"
...
SortedDatesExtraction = Struct.new(:dates, :are_sorted, :are_reverse_sorted, :log_lines)
DatesExtraction = Struct.new(:dates, :log_lines)
MaskedXpathExtraction = Struct.new(
  :links_extraction, :unfiltered_links, :log_lines, :markup_dates_extraction, :medium_markup_dates_extraction,
  :almost_markup_dates_extraction, :some_markup_dates_extraction, :maybe_url_dates_extraction,
  :title_relative_xpaths, :xpath_name
)
...
def get_masked_xpath_extraction(
  ...
  if links.length != collapsed_links.length
    log_lines << "collapsed #{links.length} -> #{collapsed_links.length} links"
  else
    log_lines << "#{collapsed_links.length} links"
  end

  filtered_links = collapsed_links
  markup_dates_extraction = SortedDatesExtraction.new(nil, nil, nil, ["Ø markup dates"])
  medium_markup_dates_extraction = DatesExtraction.new(nil, ["Ø Medium markup dates"])
  almost_markup_dates_extraction = SortedDatesExtraction.new(nil, nil, nil, ["Ø almost markup dates"])
  some_markup_dates_extraction = DatesExtraction.new(nil, ["Ø some markup dates"])
  ...
  if matching_maybe_markup_dates
    ...
    if matching_markup_dates
      ...
      if unique_links_matching_feed_count == feed_entry_links.length
        markup_dates_extraction = SortedDatesExtraction.new(
          matching_markup_dates, matching_are_sorted, matching_are_reverse_sorted,
          ["#{matching_markup_dates.length} markup dates"]
        )
      elsif unique_links_matching_feed_count >= almost_match_threshold
        almost_markup_dates_extraction = SortedDatesExtraction.new(
          matching_markup_dates, matching_are_sorted, matching_are_reverse_sorted,
          ["#{matching_markup_dates.length} almost markup dates"]
        )
      end

      some_markup_dates_extraction = DatesExtraction.new(
        matching_markup_dates, ["#{matching_markup_dates.length} some markup dates"]
      )
    end
  end
  ...
  if unique_links_matching_feed_count == feed_entry_links.length - 1
    ...
    if maybe_medium_markup_dates && maybe_medium_markup_dates.all? { |date| date != nil }
      medium_markup_dates_extraction = DatesExtraction.new(
        maybe_medium_markup_dates, ["#{maybe_medium_markup_dates.length} Medium markup dates"]
      )
    end
  end
  ...
  maybe_url_dates_extraction = DatesExtraction.new(
    maybe_url_dates, ["#{maybe_url_dates.count { |date| !date.nil? }} maybe url dates"]
  )

  MaskedXpathExtraction.new(
    links_extraction, collapsed_links, log_lines, markup_dates_extraction, medium_markup_dates_extraction,
    almost_markup_dates_extraction, some_markup_dates_extraction, maybe_url_dates_extraction,
    title_relative_xpaths, xpath_name
  )
end
